...
# AN GYARA: Sabon Redirect URI don ya dace da fzfassara.onrender.com
...
class User(UserMixin, db.Model):
    ...
    subscriptions = db.relationship('User', secondary=subscribers,primaryjoin=(subscribers.c.subscriber_id == id),secondaryjoin=(subscribers.c.channel_id == id),backref=db.backref('channel_subscribers', lazy='dynamic'),lazy='dynamic')
...
def dashboard():
    ...
    if tab not in ['home', 'shorts', 'premium', 'downloads', 'me']:
        tab = 'home'
...
def watch_video(video_id):
    ...
    if request.method == 'POST' and 'comment_text' in request.form:
        ...
        if comment_text:
            new_comment = Comment(video_id=video.id, user_id=current_user.id,text=comment_text, parent_id=int(parent_id) if parent_id else None)
...
def upload():
    if request.method == 'POST':
        ...
        if video_file and thumb_file:
            ...
            new_video = Video(user_id=current_user.id, title=title, description=description,category=category, filename=video_filename, thumbnail=thumb_filename,is_locked=is_premium_wall, is_short=is_short_tv)
...
def init_super_admin():
    ...
    if not admin_user:
        ...
        super_admin = User(name="Super Admin", email=ADMIN_EMAIL, password_hash=hashed_pw,is_admin=True, is_premium=True, bio="Main System Control Administration Account Platform")
